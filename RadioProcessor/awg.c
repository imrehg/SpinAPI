...
// Program the RadioProcessor board to run a simple demo of the shaped pulse
...
void program_and_start(float amp1, float amp2, float freq)
{
	...
	start =
    pb_inst_radio_shape(0,0,0,0, TX_ENABLE, NO_PHASE_RESET, NO_TRIGGER, USE_SHAPE, 0, 0x0F,
                  CONTINUE, 0, 10.0*us);
	...
	ret =
    pb_inst_radio_shape(0,0,0,0, TX_ENABLE, NO_PHASE_RESET, NO_TRIGGER, USE_SHAPE, 1, 0x0F,
                  CONTINUE, 0, 20.0*us);
    ...
    ret =
    pb_inst_radio_shape(0,0,0,0, TX_DISABLE, PHASE_RESET, NO_TRIGGER, NO_SHAPE, 0, 0x00,
                  BRANCH, start, 1.0*ms);
